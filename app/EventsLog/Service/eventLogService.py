...
class EventLogService():
    ...
    async def create_EventLog(self, EventLog: CreateEventLog):
        if await self.EventLog_repo.get_EventLog_by_emailCheck(EventLog.email):
            raise EventLogAlreadyExistsException(EventLog.email)
        else:
    ...
    async def get_EventLog_by_event_id(self, event_id: int):
        if await self.EventLog_repo.get_EventLog_by_event_id(event_id)==[]:
            raise EventNotFoundException(event_id)
        else:
            EventLog = await self.EventLog_repo.get_EventLog_by_event_id(event_id)
            return EventLog
